...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`users\` ADD \`title_id\` integer REFERENCES titles(id);`)
  await db.run(sql`CREATE INDEX \`users_title_idx\` ON \`users\` (\`title_id\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`titles_id\` integer REFERENCES titles(id);`)
